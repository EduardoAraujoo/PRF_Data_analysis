...
def calcular_distribuicao(df, coluna, nome_categoria):
    ...
    return [{nome_categoria: r['categoria'], "total": int(r['total']), "percentual": round((r['total']/total_geral)*100, 2)} for _, r in dist.sort_values('total', ascending=False).iterrows()]
...
def process_distribuicoes(df_input=None, save=True):
    ...
    resultado = {
        "por_tipo_acidente": calcular_distribuicao(df_master, 'tipo_acidente_x', 'tipo'),
        "por_fase_dia": calcular_distribuicao(df_master, 'fase_dia_x', 'fase'),
        "por_condicao_meteorologica": calcular_distribuicao(df_master, 'condicao_metereologica_x', 'condicao'),
