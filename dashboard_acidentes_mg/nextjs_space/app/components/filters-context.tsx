...
const FiltersContext = createContext<FiltersContextType>({ 
  filters: defaultFilters, setFilters: () => {}, queryString: '', 
  options: { anos: [], condicoes_meteorologicas: [], tipos_acidente: [], fases_dia: [] } 
});
...
export function FiltersProvider({ children }: { children: ReactNode }) {
  ...
  const [options, setOptions] = useState<Options>({ anos: [], condicoes_meteorologicas: [], tipos_acidente: [], fases_dia: [] });

  useEffect(() => {
    fetch('http://localhost:8000/api/options').then(r => r.json()).then(data => setOptions(data));
  }, []);

  const queryString = Object.entries(filters).filter(([, v]) => v !== '').map(([k, v]) => `${k}=${encodeURIComponent(v)}`).join('&');
